...
class OfferQueryService {
  ...
  async getOffersByFixtureSlug(fixtureSlug, query = {}) {
    try {
      logWithCheckpoint(
        "info",
        "Starting to fetch offers by fixture slug",
        "OFFER_016",
        { fixtureSlug, query }
      );

      // First, find the fixture by slug
      const FootballEvent = (await import("../../models/FootballEvent.js"))
        .default;
      const fixture = await FootballEvent.findOne({ slug: fixtureSlug }).lean();

      if (!fixture) {
        logWithCheckpoint("warn", "Fixture not found by slug", "OFFER_017", {
          fixtureSlug,
        });
        return null;
      }
      ...
      const {
        ...
        limit = 20,
        sortBy = "price",
        sortOrder = 1, // Ascending price for offers
        isAvailable,
        minPrice,
        maxPrice,
        currency,
      ...
      const filter = { fixtureId: fixture._id };
      ...
      const [offers, total] = await Promise.all([
        Offer.find(filter)
          ...
          .skip(skip)
          .limit(parseInt(limit))
          .populate("fixtureId", "title date type slug")
          .populate("agentId", "name whatsapp")
          .lean(),
        Offer.countDocuments(filter),
      ]);
      ...
      logWithCheckpoint(
        "info",
        "Successfully fetched offers by fixture slug",
        "OFFER_018",
        { fixtureSlug, count: offers.length, total }
      );
      ...
      logError(error, {
        operation: "getOffersByFixtureSlug",
        fixtureSlug,
        query,
      });
      throw error;
