...
async function extractPremierLeagueMatches() {
  ...
  parseString(xmlContent, (err, result) => {
    ...
    // Filter Premier League products
    const premierLeagueProducts = products.filter((product) => {
      const subsubcategories =
        product.subsubcategories && product.subsubcategories[0];
      return (
        subsubcategories &&
        subsubcategories.toLowerCase().includes("premier league 2025-2026")
      );
    });

    console.log(
      `⚽ Found ${premierLeagueProducts.length} Premier League products\n`
    );

    // Group by match (homeTeam vs awayTeam + date)
    const matchesMap = new Map();

    premierLeagueProducts.forEach((product) => {
      try {
    ...
    console.log(`✅ Found ${matches.length} unique Premier League matches\n`);
    ...
    matches.slice(0, 10).forEach((match, i) => {
      console.log(
        `${i + 1}. ${match.homeTeam} vs ${match.awayTeam} (${match.date}) - ${match.minPrice}`
      );
    });
  });
}

extractPremierLeagueMatches().catch((error) => {
