...
const OUTPUT_FILE = path.join(
  __dirname,
  "../data/p1/champions_league_matches_from_xml.json"
);

async function extractChampionsLeagueMatches() {
  ...
  parseString(xmlContent, (err, result) => {
    ...
    // Filter Champions League products
    const championsLeagueProducts = products.filter((product) => {
      const subsubcategories =
        product.subsubcategories && product.subsubcategories[0];
      return (
        subsubcategories &&
        subsubcategories.toLowerCase().includes("champions league 2025-2026")
      );
    });

    console.log(
      `⚽ Found ${championsLeagueProducts.length} Champions League products\n`
    );

    // Group by match (homeTeam vs awayTeam + date)
    const matchesMap = new Map();

    championsLeagueProducts.forEach((product) => {
      try {
    ...
    console.log(`✅ Found ${matches.length} unique Champions League matches\n`);
    ...
    matches.slice(0, 10).forEach((match, i) => {
      console.log(
        `${i + 1}. ${match.homeTeam} vs ${match.awayTeam} (${match.date}) - ${match.minPrice} EUR`
      );
    });
  });
}

extractChampionsLeagueMatches().catch((error) => {
