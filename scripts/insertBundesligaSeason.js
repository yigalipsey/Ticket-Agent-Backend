...
class BundesligaSeasonInserter {
  constructor() {
    ...
    // Date range: August 15, 2025 to end of season (May 17, 2026)
    this.startDate = "2025-08-15";
    this.endDate = "2026-05-17";
  ...
  async insertAllFixtures(fixtures) {
    try {
      console.log("========================================");
      console.log("[CHECKPOINT 3] Finding Bundesliga in database...");
      console.log("========================================\n");

      // Find Bundesliga
      const league = await this.findLeagueByApiFootballId(this.bundesligaId);

      if (!league) {
        throw new Error(
          "Bundesliga not found in database. Please run createBundesligaLeague.js first."
        );
      }
      ...
      for (const fixture of fixtures) {
        ...
        if (result) {
          ...
          const date = new Date(fixture.fixture.date)
            .toISOString()
            .split("T")[0];
          const round = this.extractRoundNumber(fixture.league.round) || "?";
          console.log(
            `${action} R${round} - ${fixture.teams.home.name} vs ${fixture.teams.away.name} (${date})`
          );
        } else {
          const date = new Date(fixture.fixture.date)
            .toISOString()
  ...
  async run() {
    try {
      console.log(
        "\n🇩🇪 BUNDESLIGA SEASON 2025/2026 - FROM AUGUST 15, 2025 🇩🇪\n"
      );
...
if (import.meta.url === `file://${process.argv[1]}`) {
  const inserter = new BundesligaSeasonInserter();
...
export default BundesligaSeasonInserter;
