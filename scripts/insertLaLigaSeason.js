...
class LaLigaSeasonInserter {
  constructor() {
    ...
    // La Liga league ID in API Football
    this.laLigaId = 140;
    this.season = 2025; // Season 2025/2026

    // Date range: October 9, 2025 to end of season (May 25, 2026)
  ...
  async fetchFixtures() {
    try {
      ...
      console.log(`League ID: ${this.laLigaId}`);
  ...
  async insertAllFixtures(fixtures) {
    try {
      console.log("========================================");
      console.log("[CHECKPOINT 3] Finding La Liga in database...");
      console.log("========================================\n");

      // Find La Liga
      const league = await this.findLeagueByApiFootballId(this.laLigaId);

      if (!league) {
        throw new Error(
          "La Liga not found in database. Please ensure it exists first."
        );
      }
  ...
  async run() {
    try {
      console.log("\n🇪🇸 LA LIGA SEASON 2025/2026 - FROM OCTOBER 9, 2025 🇪🇸\n");
...
if (import.meta.url === `file://${process.argv[1]}`) {
  const inserter = new LaLigaSeasonInserter();
...
export default LaLigaSeasonInserter;
