...
async function run() {
  ...
  try {
    ...
    // 4. Filter Bundesliga matches
    const bundesligaRecords = csvRecords.filter((r) => {
      const categoryPath = (r.categoryPath || "").toLowerCase();
      return categoryPath.includes("bundesliga 2025-2026");
    });

    console.log(
      `📊 Found ${bundesligaRecords.length} Bundesliga records in CSV\n`
    );

    // 5. Group by match and find minimum price
    const matchMap = new Map();

    bundesligaRecords.forEach((record) => {
    ...
    console.log(
      `📊 Found ${matches.length} unique Bundesliga matches with offers\n`
    );
    ...
    for (const match of matches) {
      try {
        ...
        // Determine currency (Bundesliga is EUR)
        const currency = "EUR";
