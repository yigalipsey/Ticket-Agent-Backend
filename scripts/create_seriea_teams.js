...
// Team name translations for Serie A
const teamNameTranslations = {
  "Juventus": "יובנטוס",
  "Inter": "אינטר",
  "Inter Milan": "אינטר מילאנו",
  "AC Milan": "מילאן",
  "Milan": "מילאן",
  "Napoli": "נאפולי",
  "AS Roma": "רומא",
...
// Step 1: Find existing teams for Serie A
async function findExistingTeams() {
  try {
    console.log("=".repeat(80));
    console.log("📍 STEP 1: Finding existing teams for Serie A");
    console.log("=".repeat(80));
    console.log("");
    ...
    console.log(
      `📊 Found ${teams.length} existing teams in Serie A`
    );
    console.log("");
    ...
    for (const item of apiTeams) {
      ...
      if (existingTeam) {
        ...
        if (!hasLeague) {
          ...
          if (
            ...
            const countryEn = teamData.country || "Italy";
      ...
      if (venueData && venueData.id) {
        ...
        if (!venue) {
          ...
          console.log("   Please run create_seriea_venues.js first");
          skippedTeams.push({
            name: teamData.name,
            id: apiFootballId,
