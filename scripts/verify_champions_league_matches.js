...
async function verifyMatches() {
  try {
    ...
    console.log(
      "🔍 Verifying UEFA Champions League matches from JSON file vs Database"
    );
    console.log("=".repeat(80));
    console.log("");
    ...
    console.log(
      `✅ Found league: ${championsLeague.name} (ID: ${championsLeague._id})\n`
    );
    ...
    if (!fs.existsSync(JSON_FILE)) {
      ...
      console.error(
        `   Please run fetch_champions_league_from_next_week.js first`
      );
      process.exit(1);
    }
    ...
    for (let i = 0; i < jsonMatches.length; i++) {
      ...
      console.log(
        `[CHECKPOINT ${i + 1}] Processing match ${i + 1}/${
          jsonMatches.length
        }: API ID ${apiFootballId}`
      );
      ...
      if (
        ...
        issues.push({
          ...
          db: {
            home: dbMatch.awayTeam?.name_en || dbMatch.awayTeam?.name || "N/A",
            away: dbMatch.homeTeam?.name_en || dbMatch.homeTeam?.name || "N/A",
          },
        });
      ...
      // Map known venue ID differences
      // Allianz Arena (700) -> Fußball Arena München (20732)
      // Signal Iduna Park (19381) -> BVB Stadion Dortmund (20733)
      // Orange Vélodrome (667) -> Stade Orange Vélodrome (12678)
      // Deutsche Bank Park (10491) -> Frankfurt Arena (20734)
      // Alphamega Stadium (19616) -> Stadio Stelios Kyriakides (7134)
      const venueIdMapping = {
        ...
        19616: 7134, // Alphamega Stadium -> Stadio Stelios Kyriakides
      };

      const mappedJsonVenueId = venueIdMapping[jsonVenueId] || jsonVenueId;

      if (jsonVenueId && mappedJsonVenueId !== dbVenueApiId) {
        issues.push({
          field: "venue",
          json: `${jsonMatch.venue.name} (ID: ${jsonVenueId})`,
          db: `${dbMatch.venue?.name_en || "N/A"} (ID: ${
            dbVenueApiId || "N/A"
          })`,
        });
        stats.venueMismatch++;
    ...
    const reportFile = path.resolve(
      __dirname,
      "../data/footballapi/champions_league_verification_report.json"
    );
    const report = {
    ...
    if (stats.issues.length > 0) {
      ...
      stats.issues.slice(0, 20).forEach((issue, idx) => {
        ...
        if (issue.issue === "NOT_FOUND") {
          ...
          console.log(
            `   DB: ${issue.dbMatch.homeTeam} vs ${issue.dbMatch.awayTeam}`
          );
