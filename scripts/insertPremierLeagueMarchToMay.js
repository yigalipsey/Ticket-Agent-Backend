...
class PremierLeagueMarchToMayInserter {
  constructor() {
    ...
    // Date range: March 1 to May 26, 2025
    this.startDate = "2025-03-01";
    this.endDate = "2025-05-26";
  ...
  async fetchFixtures() {
    try {
      ...
      if (response.data && response.data.response) {
        ...
        console.log(`✅ Found ${fixtures.length} fixtures (March 1 - May 26, 2025)\n`);
        return fixtures;
      } else {
  ...
  async findTeamByApiFootballId(apiFootballId, teamName) {
    try {
      ...
      console.error(
        `[ERROR] Error finding team ${teamName}:`,
        error.message
      );
      return null;
    }
  ...
  async findVenueByApiFootballId(apiFootballId, venueName) {
    try {
      ...
      console.error(
        `[ERROR] Error finding venue ${venueName}:`,
        error.message
      );
      return null;
    }
  ...
  async findLeagueByApiFootballId(apiFootballId) {
    try {
      ...
      if (!league) {
        console.log(
          `⚠️ League not found (API ID: ${apiFootballId})`
        );
        return null;
      }
    ...
    try {
      ...
      for (const fixture of fixtures) {
        ...
        if (result) {
          ...
          console.log(
            `${action} [${processedCount}/${fixtures.length}] ${fixture.teams.home.name} vs ${fixture.teams.away.name} (${date})`
          );
        } else {
          const date = new Date(fixture.fixture.date).toISOString().split("T")[0];
          console.log(
            `⏭️  [${processedCount}/${fixtures.length}] ${fixture.teams.home.name} vs ${fixture.teams.away.name} (${date}) - skipped`
          );
        }
      }
  ...
  async run() {
    try {
      console.log("\n🏴󠁧󠁢󠁥󠁮󠁧󠁿 INSERTING PREMIER LEAGUE FIXTURES (MARCH 1 - MAY 26, 2025) 🏴󠁧󠁢󠁥󠁮󠁧󠁿\n");
...
if (import.meta.url === `file://${process.argv[1]}`) {
  const inserter = new PremierLeagueMarchToMayInserter();
...
export default PremierLeagueMarchToMayInserter;
