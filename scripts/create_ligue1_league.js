...
let LIGUE1_LEAGUE_ID = null;

// Fetch league info from API-Football
async function fetchLeagueFromAPI() {
  try {
    console.log("=".repeat(80));
    console.log("📍 STEP 1: Fetching Ligue 1 information from API-Football");
    console.log("=".repeat(80));
    console.log("");

    console.log(
      `🔍 Fetching league info for Ligue 1 (API ID: ${LIGUE1_API_FOOTBALL_ID}, Season: ${CURRENT_SEASON})...`
    );
    console.log("");

    const response = await apiClient.get("/leagues", {
      params: {
        id: LIGUE1_API_FOOTBALL_ID,
...
async function createOrUpdateLeague(apiLeagueData) {
  try {
    console.log("=".repeat(80));
    console.log("📍 STEP 2: Creating/Updating Ligue 1 in database");
    console.log("=".repeat(80));
    console.log("");
    ...
    if (existing.exists) {
      ...
      if (needsUpdate) {
        ...
        if (!existing.league.externalIds?.apiFootball) {
          updateData["externalIds.apiFootball"] = LIGUE1_API_FOOTBALL_ID;
        }

        if (!existing.league.nameHe || existing.league.nameHe !== "ליג 1") {
          updateData.nameHe = "ליג 1";
        }

        if (!existing.league.countryHe || existing.league.countryHe !== "צרפת") {
          updateData.countryHe = "צרפת";
        }
    ...
    const leagueData = {
      ...
      externalIds: {
        apiFootball: LIGUE1_API_FOOTBALL_ID,
      },
    };
