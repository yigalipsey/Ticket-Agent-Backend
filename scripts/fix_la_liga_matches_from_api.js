...
const JSON_FILE = path.resolve(
  __dirname,
  "../data/footballapi/league_140_from_next_week.json"
);
...
async function fixMatches() {
  try {
    ...
    console.log("🔧 Fixing La Liga matches from API Football");
    console.log("=".repeat(80));
    console.log("");
    ...
    const laLiga = await League.findOne({
      $or: [{ slug: "la-liga" }, { slug: "laliga" }, { name: /la.?liga/i }],
    });

    if (!laLiga) {
      console.error("❌ La Liga not found in database");
      process.exit(1);
    }

    console.log(`✅ Found league: ${laLiga.name} (ID: ${laLiga._id})\n`);

    // Read JSON file
    if (!fs.existsSync(JSON_FILE)) {
      console.error(`❌ JSON file not found: ${JSON_FILE}`);
      console.error(`   Please run fetch_la_liga_from_next_week.js first`);
      process.exit(1);
    }
    ...
    for (let i = 0; i < jsonMatches.length; i++) {
      ...
      console.log(
        `[CHECKPOINT ${i + 1}] Processing ${i + 1}/${
          jsonMatches.length
        }: API ID ${apiFootballId} - ${jsonMatch.homeTeam.name} vs ${
          jsonMatch.awayTeam.name
        }`
      );
      ...
      try {
        ...
        if (!homeTeam || !awayTeam) {
          ...
          console.log(
            `   ❌ Team not found: ${
              !homeTeam ? jsonMatch.homeTeam.name : jsonMatch.awayTeam.name
            }`
          );
          continue;
        }
        ...
        if (dateDiff > 60000) {
          ...
          stats.dateUpdated++;
          console.log(
            `   📅 Will update date: ${dbDate.toISOString().split("T")[0]} → ${
              jsonDateObj.toISOString().split("T")[0]
            }`
          );
        }
        ...
        if (isReversed || updateData.date) {
          ...
          if (existingWithSlug) {
            ...
            needsUpdate = true;
            stats.slugUpdated++;
            console.log(`   🔗 Will update slug: ${dbMatch.slug} → ${newSlug}`);
          }
        }
    ...
    console.log(
      `❌ Errors: ${stats.errors.filter((e) => e.issue === "ERROR").length}`
    );
    console.log("");
    ...
    const reportFile = path.resolve(
      __dirname,
      "../data/footballapi/fix_la_liga_matches_report.json"
    );
    const report = {
