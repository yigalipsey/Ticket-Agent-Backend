...
async function fixMatches() {
  try {
    ...
    console.log("🔧 Fixing Ligue 1 matches from API Football");
    console.log("=".repeat(80));
    console.log("");
    ...
    if (!ligue1) {
      console.error("❌ Ligue 1 league not found in database");
      process.exit(1);
    }
    ...
    for (let i = 0; i < jsonMatches.length; i++) {
      ...
      try {
        ...
        if (jsonVenueId && jsonVenueId !== dbVenueApiId) {
          ...
          // Stade Jean Bouin (18861) -> Stade Charléty (12585) - but these might be different venues
          if (!newVenue) {
            if (jsonVenueId === 667) {
              // Orange Vélodrome -> Stade Orange Vélodrome
              newVenue = await Venue.findOne({
                "externalIds.apiFootball": 12678,
              }).lean();
            } else if (jsonVenueId === 18861) {
              // Stade Jean Bouin -> Stade Charléty (Paris FC's actual home)
              newVenue = await Venue.findOne({
                "externalIds.apiFootball": 12585,
              }).lean();
            }
          }
