...
class DecemberPremierLeagueInserter {
  constructor() {
  ...
  async fetchDecemberFixtures() {
    try {
      console.log("========================================");
      console.log("[CHECKPOINT 2] Fetching December fixtures from API...");
      ...
      if (response.data && response.data.response) {
        ...
        console.log(`✅ Found ${fixtures.length} fixtures in December\n`);
        return fixtures;
      } else {
        console.log("⚠️ No fixtures found in December\n");
        return [];
      }
    } catch (error) {
  ...
  async findTeamByApiFootballId(apiFootballId, teamName) {
    try {
      ...
      if (!team) {
        console.log(
          `⚠️ Team not found in database: ${teamName} (API ID: ${apiFootballId})`
        );
        return null;
      }
  ...
  async findVenueByApiFootballId(apiFootballId, venueName) {
    try {
      ...
      if (!venue) {
        console.log(
          `⚠️ Venue not found in database: ${venueName} (API ID: ${apiFootballId})`
        );
        return null;
      }
  ...
  async findLeagueByApiFootballId(apiFootballId) {
    try {
      ...
      if (!league) {
        console.log(
          `⚠️ League not found in database (API ID: ${apiFootballId})`
        );
        return null;
      }
    ...
    try {
      ...
      for (const fixture of fixtures) {
        ...
        if (result) {
          ...
          console.log(
            `${action} [${processedCount}/${fixtures.length}] ${fixture.teams.home.name} vs ${fixture.teams.away.name} - ${result.action}`
          );
        } else {
          console.log(
            `⏭️  [${processedCount}/${fixtures.length}] ${fixture.teams.home.name} vs ${fixture.teams.away.name} - skipped`
          );
        }
      }
  ...
  displaySummary() {
    ...
    if (this.stats.errors.length > 0) {
      ...
      this.stats.errors.forEach((error, index) => {
        console.log(`\n[Error #${index + 1}]`);
        console.log(`Fixture ID: ${error.fixtureId}`);
        console.log(`Reason: ${error.reason}`);
        if (error.homeTeam) console.log(`Home Team: ${error.homeTeam}`);
        if (error.awayTeam) console.log(`Away Team: ${error.awayTeam}`);
  ...
  async run() {
    try {
      console.log(
        "\n🏴󠁧󠁢󠁥󠁮󠁧󠁿 INSERTING DECEMBER PREMIER LEAGUE FIXTURES TO MONGODB 🏴󠁧󠁢󠁥󠁮󠁧󠁿\n"
      );
...
if (import.meta.url === `file://${process.argv[1]}`) {
  const inserter = new DecemberPremierLeagueInserter();
...
export default DecemberPremierLeagueInserter;
