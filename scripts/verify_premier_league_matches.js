...
const JSON_FILE = path.resolve(
  __dirname,
  "../data/footballapi/league_39_from_next_week.json"
);
...
async function verifyMatches() {
  try {
    ...
    console.log("🔍 Verifying Premier League matches from JSON file vs Database");
    console.log("=".repeat(80));
    console.log("");
    ...
    console.log(`✅ Found league: ${premierLeague.name} (ID: ${premierLeague._id})\n`);

    // Read JSON file
    if (!fs.existsSync(JSON_FILE)) {
      console.error(`❌ JSON file not found: ${JSON_FILE}`);
      console.error(`   Please run fetch_premier_league_from_next_week.js first`);
      process.exit(1);
    }
    ...
    const reportFile = path.resolve(
      __dirname,
      "../data/footballapi/premier_league_verification_report.json"
    );
    const report = {
