...
async function findTeam(teamName, leagueId) {
  ...
  // Try partial match
  team = await Team.findOne({
    name_en: new RegExp(teamName.replace(/FC|United|City|CP/g, "").trim(), "i"),
    leagueIds: leagueId,
  }).lean();
...
async function run() {
  ...
  try {
    ...
    // Find P1 supplier and Champions League
    ...
    for (let i = 0; i < matches.length; i++) {
      ...
      // Find teams
      const homeTeam = await findTeam(homeTeamName, championsLeague._id);
      const awayTeam = await findTeam(awayTeamName, championsLeague._id);
    ...
    const outputFile = path.join(
      __dirname,
      "../data/p1/champions_league_price_updates.json"
    );
