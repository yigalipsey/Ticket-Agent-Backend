...
async function fetchMatchesFromAPI() {
  try {
    ...
    console.log(
      `🔍 Fetching fixtures for Ligue 1 (ID: ${LIGUE1_API_FOOTBALL_ID}, Season: ${CURRENT_SEASON})...`
    );
    console.log("");
...
async function createMatches(apiFixtures, league, teams, venues) {
  try {
    ...
    for (let i = 0; i < apiFixtures.length; i++) {
      ...
      try {
        ...
        const matchData = {
          ...
          roundNumber: null,
          slug: slug,
          tags: ["Ligue 1", CURRENT_SEASON.toString()],
          externalIds: {
    ...
    console.log(`📊 Total Ligue 1 matches in DB: ${totalMatches}`);
...
async function main() {
  try {
    ...
    console.log(`✅ Loaded ${teams.length} teams for Ligue 1`);
    console.log("");

    if (teams.length === 0) {
      console.log("❌ No teams found for Ligue 1");
      console.log("   Please run create_ligue1_teams.js first");
      await databaseConnection.disconnect();
      process.exit(1);
    }
