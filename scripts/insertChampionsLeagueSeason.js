...
class ChampionsLeagueSeasonInserter {
  constructor() {
    ...
    // Champions League ID in API Football
    this.championsLeagueId = 2;
    this.season = 2025; // Season 2025/2026

    // Date range: October 9, 2025 to end of season (June 30, 2026)
  ...
  async fetchFixtures() {
    try {
      ...
      console.log(`League ID: ${this.championsLeagueId}`);
  ...
  extractRoundNumber(roundString) {
    ...
    // Champions League rounds format examples:
    // "League Stage - 1", "Play-offs - Round of 16", "Final"
    const matchLeague = roundString.match(/League Stage - (\d+)/);
    if (matchLeague) return parseInt(matchLeague[1]);

    const matchPlayoff = roundString.match(/Round of (\d+)/);
    if (matchPlayoff) return parseInt(matchPlayoff[1]);

    if (roundString.includes("Final")) return 999;
    if (roundString.includes("Semi")) return 4;
    if (roundString.includes("Quarter")) return 8;
  ...
  async insertOrUpdateFixture(fixture, league) {
    try {
      ...
      // If venue not found by API ID, use home team's venue
      if (!venue) {
        venue = homeTeam.venueId;
        if (typeof venue === "object" && venue._id) {
          venue = venue._id;
        }
        console.log(
          `   ℹ️  Using home team venue for: ${fixture.teams.home.name} vs ${fixture.teams.away.name}`
        );
      }
  ...
  async insertAllFixtures(fixtures) {
    try {
      console.log("========================================");
      console.log("[CHECKPOINT 3] Finding Champions League in database...");
      console.log("========================================\n");

      // Find Champions League
      const league = await this.findLeagueByApiFootballId(
        this.championsLeagueId
      );

      if (!league) {
        throw new Error(
          "Champions League not found in database. Please ensure it exists first."
        );
      }
      ...
      for (const fixture of fixtures) {
        ...
        if (processedCount % 10 === 0 || processedCount === totalFixtures) {
          ...
          console.log(
            `\n📊 [${processedCount}/${totalFixtures}] Progress: ${percentage}% | ✅ ${this.stats.inserted} | 🔄 ${this.stats.updated} | ⏭️ ${this.stats.skipped}`
          );
        }
        ...
        if (result) {
          ...
          console.log(
            `${action} ${round} - ${fixture.teams.home.name} vs ${fixture.teams.away.name} (${date})`
          );
        } else {
          const date = new Date(fixture.fixture.date)
            .toISOString()
  ...
  async run() {
    try {
      console.log("\n🏆 UEFA CHAMPIONS LEAGUE SEASON 2025/2026 🏆\n");
...
if (import.meta.url === `file://${process.argv[1]}`) {
  const inserter = new ChampionsLeagueSeasonInserter();
...
export default ChampionsLeagueSeasonInserter;
