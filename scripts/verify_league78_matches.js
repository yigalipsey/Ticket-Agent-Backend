...
async function verifyMatches() {
  try {
    ...
    console.log("🔍 Verifying League 78 matches from JSON file vs Database");
    console.log("=".repeat(80));
    console.log("");
    ...
    for (let i = 0; i < jsonMatches.length; i++) {
      ...
      if (jsonHomeTeamId !== dbHomeTeamApiId) {
        issues.push({
          field: "homeTeam",
      ...
      if (issues.length === 0) {
        ...
        console.log(`   ⚠️  Found ${issues.length} mismatch(es):`);
        issues.forEach((issue) => {
          console.log(`      - ${issue.field}: JSON="${issue.json}" vs DB="${issue.db}"`);
        });
      }
    }
    ...
    const reportFile = path.resolve(
      __dirname,
      "../data/footballapi/league_78_verification_report.json"
    );
    const report = {
    ...
    if (stats.issues.length > 0) {
      console.log("=".repeat(80));
      console.log("⚠️  SAMPLE ISSUES (first 10):");
      console.log("=".repeat(80));
      ...
      if (stats.issues.length > 10) {
        console.log(`\n... and ${stats.issues.length - 10} more issues`);
      }
      console.log("");
    }
