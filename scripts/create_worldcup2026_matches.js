...
async function fetchMatchesFromAPI() {
  try {
    ...
    if (
      ...
      console.log(
        "   - The World Cup 2026 fixtures are not yet available in the API"
      );
      console.log("   - The season/league ID might be different");
      console.log("");
      return [];
    }
    ...
    // Display first few fixtures for preview
    if (fixturesResponse.data.response.length > 0) {
      console.log("📋 Sample fixtures:");
      fixturesResponse.data.response.slice(0, 10).forEach((fixture, index) => {
        const fixtureData = fixture.fixture;
        const teamsData = fixture.teams;
        console.log(
          `${index + 1}. ${teamsData.home.name} vs ${teamsData.away.name}`
        );
        console.log(`   Date: ${fixtureData.date}`);
        console.log(`   Venue: ${fixture.venue?.name || "N/A"}`);
        console.log(`   Round: ${fixture.league?.round || "N/A"}`);
        console.log(`   Status: ${fixtureData.status?.long || "N/A"}`);
        console.log("");
      });
      if (fixturesResponse.data.response.length > 10) {
        console.log(
          `   ... and ${
            fixturesResponse.data.response.length - 10
          } more fixtures`
        );
        console.log("");
      }
...
async function createMatches(apiFixtures, league, teams, venues) {
  try {
    ...
    console.log(
      `📊 Loaded ${teams.length} teams and ${venues.length} venues for lookup`
    );
    console.log("");
    ...
    for (let i = 0; i < apiFixtures.length; i++) {
      ...
      try {
        ...
        const matchData = {
          ...
          roundNumber: null,
          slug: slug,
          tags: ["World Cup 2026", CURRENT_SEASON.toString()],
          externalIds: {
        ...
        if (existingMatch) {
          ...
          if (fixtureData.id && !existingMatch.externalIds?.apiFootball) {
    ...
    console.log(`📊 Total World Cup 2026 matches in DB: ${totalMatches}`);
...
async function main() {
  try {
    ...
    // Step 1: Fetch matches from API first (to check if they exist)
    const apiFixtures = await fetchMatchesFromAPI();

    if (apiFixtures.length === 0) {
      console.log("=".repeat(80));
      console.log("ℹ️  No matches found in API for World Cup 2026");
      console.log("=".repeat(80));
      await databaseConnection.disconnect();
      process.exit(0);
    }
    ...
    console.log(`✅ Loaded ${teams.length} teams for World Cup 2026`);
    console.log("");

    if (teams.length === 0) {
      console.log("=".repeat(80));
      console.log("⚠️  No teams found in database for World Cup 2026");
      console.log(
        `   Found ${apiFixtures.length} matches in API, but need teams first`
      );
      console.log("   Please run create_worldcup2026_teams.js first");
      console.log("=".repeat(80));
      await databaseConnection.disconnect();
      process.exit(0);
    }
