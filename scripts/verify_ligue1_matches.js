...
const JSON_FILE = path.resolve(
  __dirname,
  "../data/footballapi/league_61_from_next_week.json"
);
...
async function verifyMatches() {
  try {
    ...
    console.log("🔍 Verifying Ligue 1 matches from JSON file vs Database");
    console.log("=".repeat(80));
    console.log("");
    ...
    // Find Ligue 1 league
    const ligue1 = await League.findOne({
      $or: [{ slug: "ligue-1" }, { name: /ligue.?1/i }],
    });

    if (!ligue1) {
      console.error("❌ Ligue 1 not found in database");
      process.exit(1);
    }

    console.log(`✅ Found league: ${ligue1.name} (ID: ${ligue1._id})\n`);

    // Read JSON file
    if (!fs.existsSync(JSON_FILE)) {
      console.error(`❌ JSON file not found: ${JSON_FILE}`);
      console.error(`   Please run fetch_ligue1_from_next_week.js first`);
      process.exit(1);
    }
    ...
    for (let i = 0; i < jsonMatches.length; i++) {
      ...
      // Stade Jean Bouin (18861) -> Stade Charléty (12585)
      const venueIdMapping = {
        667: 12678,   // Orange Vélodrome -> Stade Orange Vélodrome
        18861: 12585, // Stade Jean Bouin -> Stade Charléty
      };
    ...
    const reportFile = path.resolve(
      __dirname,
      "../data/footballapi/ligue1_verification_report.json"
    );
    const report = {
